...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("/home/codec_agences/logs/process_afp.log"),logging.StreamHandler()]
)
...
def get_label(text,agency_folder):
    ...
    if agency_folder == "AFPECO":
        match = re.search(r"ECF(\d+)", text)
    elif agency_folder == "AFPLOCAL":
        match = re.search(r"FRS(\d+)", text)
    else:
        match = re.search(r"MOA(\d+)", text)
...
def parse_txt(file_path,agency_folder):
    ...
    label = get_label(lines[0].strip() if agency_folder == "AFPLOCAL" else lines[3].strip(),agency_folder)
    #slug =  lines[4].strip()
    slug = lines[1].strip() if agency_folder == "AFPLOCAL" else lines[4].strip()
    title_lines = []

    
    if agency_folder == "AFPLOCAL":
        for line in lines[2:]:
            if not line.strip():
                break
